"""
OpenAI API client wrapper for risk assessment and replies.

Uses OpenAI's GPT models for trauma-informed responses.

To use:
1. Set in backend/.env: OPENAI_API_KEY=sk-or-v1-...
...
class OpenAIClient:
    ...
    async def generate(self, message: str, language: str, name: str = "") -> ChatResult:
        """Generate a response using OpenAI API. Falls back to heuristic if API unavailable."""
        if not self.api_key or not self.client:
            print("⚠️  OPENAI_API_KEY not set — using offline heuristic mode")
        ...
        try:
            ...
            print(f"⚠️  OpenAI API error: {e} — using offline fallback")
            return self._stub_response(message, language, name)

    def _parse_json_response(self, text: str, language: str, name: str = "") -> ChatResult:
        """Parse JSON response from OpenAI, with graceful fallback."""
        clean = text.strip()
        if clean.startswith("```"):
            clean = clean.split("```")[1]
    ...
    def _stub_response(self, message: str, language: str, name: str = "") -> ChatResult:
        ...
        Used when OPENAI_API_KEY is not set or API is unavailable.
        """
        m = message.lower()
